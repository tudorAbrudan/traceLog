  ...
  {#if server?.host === 'localhost'}
    <div class="docker-logs-panel">
      <div class="card-head docker-head">
        ...
        <span class="hint">Runs <code>docker logs</code> on this machine (local server only).</span>
      </div>
...
<style>
  ...
  .docker-head { flex-direction: column; align-items: flex-start; gap: 0.25rem; margin-bottom: 0.75rem; }
  .docker-head .hint { font-size: 0.72rem; color: var(--text-muted); font-weight: 400; }
  .docker-head code { font-size: 0.7rem; background: var(--bg-hover); padding: 1px 4px; border-radius: 4px; }
  .docker-empty { font-size: 0.85rem; color: var(--text-muted); margin: 0; }
  .docker-toolbar { display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: center; margin-bottom: 0.5rem; }
  ...
  .btn-logs {
    ...
    border-radius: 8px; padding: 0.45rem 1rem; font-size: 0.8rem; font-weight: 600; cursor: pointer;
  }
  .btn-logs:disabled { opacity: 0.5; cursor: not-allowed; }
  .docker-log-out, .docker-err {
    margin: 0; max-height: 360px; overflow: auto;
    background: var(--bg-primary); border: 1px solid var(--border);
    border-radius: 8px; padding: 0.75rem; font-size: 0.72rem; line-height: 1.45;
    white-space: pre-wrap; word-break: break-all;
  }
  .docker-err { color: var(--danger); border-color: #f8514944; }
</style>
